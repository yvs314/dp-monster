...
struct t_slnRequestState
{
	...
	std::pair<e_parState, t_Direction> D{ e_parState::absent,BWD };//-d FWD | BWD default to BWD
...
//---------------|-H|:BREADTH--------------------------------------/
	...
	//"--UB" key found; attempt to parse it (its "argument" is in keyArg.first )
	{
		...
		if (!is_number(keyArg.first))
		{
			...
			out.UB = std::make_pair(e_parState::present, std::stoul(keyArg.first));
	}
	//-------------------next--parameter----------------------------------------/
	//----------------|-t|:PROBLEM--TYPE--CODE-------------------------------/
		...
		//"-d" key found; attempt to parse it (its "argument" is in keyArg.first)
	{
		...
		out.D = std::make_pair(e_parState::present, iDirn);
	}
